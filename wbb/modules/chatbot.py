...
# decorator shim: if userbot is absent, don't register handlers
...
async def chatbot_talk_ubot(_, message: Message):
    ...
    if message.chat.id not in db["userbot"]:
        return
    username = ("@" + str(USERBOT_USERNAME)) if USERBOT_USERNAME else ""
    if message.reply_to_message:
        if not message.reply_to_message.from_user:
            return
        if (
            message.reply_to_message.from_user.id != USERBOT_ID
            and username not in (message.text or "")
        ):
            return
    else:
        if username not in (message.text or ""):
            return
    await type_and_send(message)
